...
# ============================
# 6B. Oversample Spike Samples in Training Set
# ============================
...
train_sampler = PerUserBatchSampler(create_user_datasets(augmented_train_samples), batch_size=BATCH_SIZE)
...
val_sampler = PerUserBatchSampler(create_user_datasets(val_samples), batch_size=BATCH_SIZE)
...
class ForecastingModel(nn.Module):
    ...
    Attention mechanisms, Fusion layers, Decoders, and a Classification head for spike detection.
    """
    def __init__(self):
        super(ForecastingModel, self).__init__()
        # CNN Encoder for BPM
        self.bpm_cnn = nn.Sequential(
            nn.Conv1d(in_channels=INPUT_WINDOWS, out_channels=32, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Dropout(0.3),
    ...
    def forward(self, bpm_input, steps_input, hour, day_of_week):
        ...
        """
        # Reshape BPM input: [batch_size, INPUT_WINDOWS, WINDOW_SIZE]
        bpm_seq = bpm_input  # Shape: [B, INPUT_WINDOWS, WINDOW_SIZE]
        
        # CNN Encoder for BPM
        bpm_cnn_output = self.bpm_cnn(bpm_seq)  # Shape: [B, 64, WINDOW_SIZE]
        bpm_cnn_output = bpm_cnn_output.permute(0, 2, 1)  # Shape: [B, WINDOW_SIZE, 64]
        
        # LSTM Encoder for BPM
        bpm_lstm_output, _ = self.bpm_lstm(bpm_cnn_output)  # Shape: [B, WINDOW_SIZE, 128]
        ...
        # Reshape Steps input similarly: [B, INPUT_WINDOWS, WINDOW_SIZE]
        steps_seq = steps_input  # Shape: [B, INPUT_WINDOWS, WINDOW_SIZE]
        
        # CNN Encoder for Steps
        steps_cnn_output = self.steps_cnn(steps_seq)  # Shape: [B, 64, WINDOW_SIZE]
        steps_cnn_output = steps_cnn_output.permute(0, 2, 1)  # Shape: [B, WINDOW_SIZE, 64]
        
        # LSTM Encoder for Steps
        steps_lstm_output, _ = self.steps_lstm(steps_cnn_output)  # Shape: [B, WINDOW_SIZE, 128]
...
class CombinedLoss(nn.Module):
    def __init__(self):
        ...
        self.regression_loss = nn.MSELoss(reduction='none')        # For step counts
        self.classification_loss = nn.BCELoss(reduction='none')    # For spike detection
    
    def forward(self, bpm_pred, bpm_target, steps_pred, steps_target, spike_prob, spike_label, step_weights, spike_weight=1.0):
        # Regression Loss for BPM
        loss_bpm = self.regression_loss(bpm_pred, bpm_target)     # Shape: [B, WINDOW_SIZE]
        loss_bpm = loss_bpm.mean(dim=1)                           # Shape: [B]
        
        # Safeguard: Ensure step_weights is a tensor
        if not isinstance(step_weights, torch.Tensor):
            print("Warning: step_weights is not a tensor. Converting to tensor.")
            step_weights = torch.as_tensor(step_weights, dtype=torch.float32, device=bpm_pred.device)
        
        # Ensure step_weights is a float tensor and same dtype as loss_bpm
        step_weights = step_weights.float().to(loss_bpm.dtype)
        
        # Apply weights
        loss_bpm = loss_bpm * step_weights                        # Shape: [B]
        
        # Regression Loss for Steps
        loss_steps = self.regression_loss(steps_pred, steps_target)  # Shape: [B, WINDOW_SIZE]
        loss_steps = loss_steps.mean(dim=1)                            # Shape: [B]
        loss_steps = loss_steps * step_weights                         # Shape: [B]
        
        # Classification Loss for Spike Detection
        loss_spike = self.classification_loss(spike_prob, spike_label)  # Shape: [B]
        loss_spike = loss_spike * spike_weight                          # Shape: [B]
        
        # Combine all losses
        total_loss = loss_bpm + loss_steps + loss_spike
        return total_loss.mean()  # Return scalar
