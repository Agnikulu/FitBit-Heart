...
Cross-modal masking: each head sees ONLY the *other* modality
for the current window, preventing data-leakage / shortcut-learning.
"""
...
class SSLForecastingModel(nn.Module):
    ...
    def __init__(self, *, window_size: int, predict_windows: int, cfg_model: dict):
        super().__init__()
        ...
        # CNN-GRU encoders for past history
        ...
        # current-window embedding (16 → token)
        ...
        # two independent Transformer blocks (one per target modality)
        def _attn_block():
    ...
    def forward(
        self,
        bpm_in: torch.Tensor,         # [B, 2, W]
        steps_in: torch.Tensor,       # [B, 2, W]
        cur_bpm: torch.Tensor,        # [B, P, W]
        cur_steps: torch.Tensor       # [B, P, W]
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        B, P = bpm_in.size(0), self.predict_windows
        past_summary = self._extract_past_summary(bpm_in, steps_in)  # [B, 2H]

        # *** CROSS-MODAL MASKING ***
        #   – predicting BPM  → see ONLY current-steps
        #   – predicting Steps→ see ONLY current-BPM
        feats_bpm   = [self.agg_current_steps(cur_steps[:, i, :]) for i in range(P)]
        feats_steps = [self.agg_current_bpm  (cur_bpm  [:, i, :]) for i in range(P)]

        tok_bpm   = self._make_tokens(feats_bpm)
        tok_steps = self._make_tokens(feats_steps)

        def _add_pos(tokens):
            seq = torch.cat([past_summary.unsqueeze(1), tokens], dim=1)  # [B, P+1, E]
            pos = self.pos_emb(torch.arange(P + 1, device=seq.device))[None, :, :]
            return seq + pos
        ...
        def _xformer(seq, attn, ln1, ffn, ln2):
            a, _ = attn(seq, seq, seq)   # self-attention
            x = ln1(seq + a)
            x = ln2(x + ffn(x))
            return x

        out_bpm   = _xformer(seq_bpm,   self.attn_bpm,   self.ln1_bpm,   self.ffn_bpm,   self.ln2_bpm)
        out_steps = _xformer(seq_steps, self.attn_steps, self.ln1_steps, self.ffn_steps, self.ln2_steps)

        pred_bpm   = self.fusion_bpm  (out_bpm  [:, 1:, :])  # drop past token
        pred_steps = self.fusion_steps(out_steps[:, 1:, :])

        return pred_bpm, pred_steps

class PersonalizedForecastingModel(SSLForecastingModel):
    """Fine-tuning wrapper (inherits structure from SSL backbone)."""
    pass
